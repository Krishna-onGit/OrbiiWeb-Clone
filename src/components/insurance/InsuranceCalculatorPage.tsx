...
export default function InsuranceCalculatorPage({ onBack, onSelectInsurance, onSelectProduct, onSelectPage }: InsuranceCalculatorPageProps) {
  useEffect(() => {
    initializeSpotlightEffects('.category-card', {
      size: 250,
      intensity: 0.1,
      color: '255, 255, 255'
    });
    ...
    <div className="min-h-screen bg-transparent text-white pt-24 pb-12">
      ...
      <div className="container mx-auto px-4 md:px-8 mb-6">
        <div className="flex items-center gap-4 mb-4">
          <button
            onClick={onBack}
            className="p-1 px-3 rounded-lg border border-white/10 hover:border-white/20 bg-white/5 hover:bg-white/10 transition-all flex items-center gap-2"
          >
            <ArrowLeft className="w-4 h-4" />
            <span className="text-xs">Back to Home</span>
          </button>
        </div>
        <h1 className="text-4xl md:text-5xl font-thin mb-1">Insurance Products</h1>
        <p className="text-white/60 text-base md:text-lg">Explore our comprehensive insurance solutions</p>
      </div>

      {/* Quick Navigation - Below the text */}
      <section className="max-w-7xl mx-auto px-5 sm:px-6 md:px-8 py-2 mb-4">
        <div className="sticky top-28 z-40 bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl p-4 sm:p-5 shadow-2xl">
          <h2 className="text-white/40 text-[10px] uppercase tracking-widest mb-3 text-center font-semibold">Quick Navigation</h2>
          <div className="flex flex-nowrap items-center justify-start md:justify-center gap-2 overflow-x-auto no-scrollbar pb-1">
            {['Life Insurance', 'Health Insurance', 'Motor Insurance', 'Investment Plans', 'Other Insurance', 'Calculators', 'Compare Plans', 'Tax Benefits'].map((item, idx) => (
              <a
                ...
                className="whitespace-nowrap px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-white/80 text-[12px] font-medium hover:bg-brand/10 hover:border-brand/30 hover:text-brand transition-all duration-300"
              >
                {item}
              </a>
            ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 md:gap-6">
          {insuranceCategories.map((category) => {
            ...
            return (
              <div
                key={category.id}
                className="category-card spotlight-effect rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md p-6 hover:border-white/20 hover:bg-white/10 transition-all duration-300"
              >
                {/* Category Header */}
